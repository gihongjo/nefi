...
┌─────────────────────────────────────────┐
│  Web UI (Svelte 5)                      │
│  ├── Dashboard  — 엔드포인트 통계 실시간 │
│  ├── Topology   — 서비스 그래프 실시간   │
│  └── Settings   — 테마 & 레이아웃 설정  │
└─────────────────────────────────────────┘
```
...
- [x] **nefi-server**
  ...
  - 인메모리 링 버퍼 스토어 (최근 50만 이벤트)
  - 슬라이딩 윈도우 집계기 (1초 단위 버킷, 최대 5분)
  - 1초 주기로 집계 결과를 브로드캐스트하는 WebSocket 허브
  - REST API: `GET /stats`, `GET /events`, `GET /topology`

- [x] **Web UI**
  - Dashboard: 엔드포인트별 요청 수·성공률·레이턴시 실시간 표시
  - Topology: 성공률에 따라 색상이 변하는 인터랙티브 서비스 그래프 (cytoscape.js + dagre)
